...
namespace JP.KShoji.Driver.Midi.Thread
{
	...
	public sealed class MidiDeviceConnectionWatcher
	{
		...
		{
			deviceGrantQueue = new List<UsbDevice>();
			isGranting = false;
		...
		{
			private static readonly string USB_PERMISSION_GRANTED_ACTION = "jp.kshoji.driver.midi.USB_PERMISSION_GRANTED_ACTION";
			...
			public override void OnReceive(Context context, Intent intent)
			{
				...
				if (MidiDeviceConnectionWatcher.UsbMidiGrantedReceiver.USB_PERMISSION_GRANTED_ACTION
					.Equals(action))
				{
					bool granted = intent.GetBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false);
					if (granted)
					{
		...
		private sealed class MidiDeviceConnectionWatchThread : Sharpen.Thread
		{
			...
			public override void Run()
			{
				...
				while (this.stopFlag == false)
				{
					...
					{
						if (!this._enclosing.deviceGrantQueue.IsEmpty() && !this._enclosing.isGranting)
						{
							this._enclosing.isGranting = true;
							UsbDevice device = this._enclosing.deviceGrantQueue.Remove();
							...
							this.context.RegisterReceiver(new MidiDeviceConnectionWatcher.UsbMidiGrantedReceiver
								(this, device.GetDeviceName(), device, this.deviceAttachedListener), new IntentFilter
			...
			internal void CheckConnectedDevices()
			{
				lock (this)
				{
					Dictionary<string, UsbDevice> deviceMap = this.usbManager.GetDeviceList();
					...
					foreach (string deviceName in deviceMap.Keys)
					{
						...
						if (!this.connectedDeviceNameSet.Contains(deviceName))
						{
							...
							if (midiInterfaces.Count > 0)
							{
								lock (this._enclosing.deviceGrantQueue)
								{
									this._enclosing.deviceGrantQueue.AddItem(device);
								}
							}
						}
					}
					...
					foreach (string deviceName_1 in this.connectedDeviceNameSet)
					{
						if (!deviceMap.ContainsKey(deviceName_1))
						{
							...
							Log.D(Constants.TAG, "deviceName:" + deviceName_1 + ", device:" + device + " detached."
								);
							if (device != null)
							{
